...
- **路径安全**：所有 Write/Glob/Read 操作的知识目录路径必须以 `.reference/wiki/<仓库名>/` 开头，绝对禁止使用系统绝对路径（如 `/Users/...`、`C:\...`、`/home/...`）。违反此规则会导致跨项目/跨机器不可用。
- 文件引用使用仓库相对路径（如 `storage/storage.go`），禁止包含任何系统绝对路径。
